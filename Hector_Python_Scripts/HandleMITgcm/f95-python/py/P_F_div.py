...
for ii in range(0,ku-1):
    ...
    # handling the nan's
    dv = divergence.d(u,v,dx[cor_sw:cor_ne,cor_ws:cor_en],dy[cor_sw:cor_ne,cor_ws:cor_en],raz[cor_sw:cor_ne,cor_ws:cor_en],m,n)
    dv = dv/f
    #nn = len(data)
    RMS[ii] = rms.var(dv[~np.isnan(dv)])
   # Coriolis freq
#f = 0.000037
#levels = np.arange(-2,2,0.1)
#cs = plt.contourf(dv/f,levels,cmap=cm.jet)
#cb = plt.colorbar(cs,extend='both',pad=0.05)
#plt.show()
scipy.io.savemat('div_rec_c.mat',dict(RMS=RMS))
