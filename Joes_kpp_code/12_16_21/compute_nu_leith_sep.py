...
def compute_nu_leith_sep_diag(U,V,thknss,dxc,dyc,dxu,dyu,dxv,dyv,dxq,dyq,ugrid,vgrid,pgrid,post_taper=np.array([1,1,0]),
                     trim_ml=0,zoversamp=1,prd=np.array([1,1,1])):

    (dU_D,dV_D,dU_Z,dV_Z,ViscA4D,ViscA4Z) = compute_nu_leith_sep(U,V,thknss,dxc,dyc,dxu,dyu,dxv,dyv,dxq,dyq,ugrid,vgrid,pgrid,post_taper,trim_ml,zoversamp,prd);

    return (dU_D,dV_D,dU_Z,dV_Z,ViscA4D,ViscA4Z);

def compute_nu_leith_sep_diag_u(U,V,thknss,dxc,dyc,dxu,dyu,dxv,dyv,dxq,dyq,ugrid,vgrid,pgrid,post_taper=np.array([1,1,0]),
                     trim_ml=0,zoversamp=1,prd=np.array([1,1,1])):

    (dU_D,dV_D,dU_Z,dV_Z,_,_) = compute_nu_leith_sep(U,V,thknss,dxc,dyc,dxu,dyu,dxv,dyv,dxq,dyq,ugrid,vgrid,pgrid,post_taper,trim_ml,zoversamp,prd);
...
def compute_nu_leith_sep_viscA4(U,V,thknss,dxc,dyc,dxu,dyu,dxv,dyv,dxq,dyq,ugrid,vgrid,pgrid,post_taper=np.array([1,1,0]),
                     trim_ml=0,zoversamp=1,prd=np.array([1,1,1])):

    (_,_,_,_,viscA4_D,viscA4_Z) = compute_nu_leith_sep(U,V,thknss,dxc,dyc,dxu,dyu,dxv,dyv,dxq,dyq,ugrid,vgrid,pgrid,post_taper,trim_ml,zoversamp,prd);

    return (viscA4_D,viscA4_Z);

def compute_nu_leith_sep_k(U,V,thknss,dxc,dyc,dxu,dyu,dxv,dyv,dxq,dyq,ugrid,vgrid,pgrid,post_taper=np.array([1,1,0]),
                     trim_ml=0,zoversamp=1,prd=np.array([1,1,1])):

    (dU_D,dV_D,dU_Z,dV_Z,_,_) = compute_nu_leith_sep(U,V,thknss,dxc,dyc,dxu,dyu,dxv,dyv,dxq,dyq,ugrid,vgrid,pgrid,post_taper,trim_ml,zoversamp,prd);
    (T_D,k_out) = compute_nu_T_k(dU_D,dV_D,U,V,thknss,dxu,dyu,dxv,dyv,dxc,dyc,ugrid,vgrid,post_taper,trim_ml,zoversamp,prd);
    (T_Z,k_out) = compute_nu_T_k(dU_Z,dV_Z,U,V,thknss,dxu,dyu,dxv,dyv,dxc,dyc,ugrid,vgrid,post_taper,trim_ml,zoversamp,prd);

    return (T_D,T_Z,k_out);

def compute_nu_leith_sep_m(U,V,thknss,dxc,dyc,dxu,dyu,dxv,dyv,dxq,dyq,ugrid,vgrid,pgrid,post_taper=np.array([1,1,0]),
                     trim_ml=0,zoversamp=1,prd=np.array([1,1,1])):

    (dU_D,dV_D,dU_Z,dV_Z,_,_) = compute_nu_leith_sep(U,V,thknss,dxc,dyc,dxu,dyu,dxv,dyv,dxq,dyq,ugrid,vgrid,pgrid,post_taper,trim_ml,zoversamp,prd);
    (T_D,m_out) = compute_nu_T_m(dU_D,dV_D,U,V,thknss,dxu,dyu,dxv,dyv,dxc,dyc,ugrid,vgrid,post_taper,trim_ml,zoversamp,prd);
    (T_Z,m_out) = compute_nu_T_m(dU_Z,dV_Z,U,V,thknss,dxu,dyu,dxv,dyv,dxc,dyc,ugrid,vgrid,post_taper,trim_ml,zoversamp,prd);

    return (T_D,T_Z,m_out);

def compute_nu_leith_sep_k_z(U,V,thknss,dxc,dyc,dxu,dyu,dxv,dyv,dxq,dyq,ugrid,vgrid,pgrid,post_taper=np.array([1,1,0]),
                     trim_ml=0,zoversamp=1,prd=np.array([1,1,1])):

    (dU_D,dV_D,dU_Z,dV_Z,_,_) = compute_nu_leith_sep(U,V,thknss,dxc,dyc,dxu,dyu,dxv,dyv,dxq,dyq,ugrid,vgrid,pgrid,post_taper,trim_ml,zoversamp,prd);
    (T_D,k_out) = compute_nu_T_k_z(dU_D,dV_D,U,V,thknss,dxu,dyu,dxv,dyv,dxc,dyc,ugrid,vgrid,post_taper,trim_ml,zoversamp,prd);
    (T_Z,k_out) = compute_nu_T_k_z(dU_Z,dV_Z,U,V,thknss,dxu,dyu,dxv,dyv,dxc,dyc,ugrid,vgrid,post_taper,trim_ml,zoversamp,prd);

    return (T_D,T_Z,k_out);

def compute_nu_leith_sep_m_xy(U,V,thknss,dxc,dyc,dxu,dyu,dxv,dyv,dxq,dyq,ugrid,vgrid,pgrid,post_taper=np.array([1,1,0]),
                     trim_ml=0,zoversamp=1,prd=np.array([1,1,1])):

    (dU_D,dV_D,dU_Z,dV_Z,_,_) = compute_nu_leith_sep(U,V,thknss,dxc,dyc,dxu,dyu,dxv,dyv,dxq,dyq,ugrid,vgrid,pgrid,post_taper,trim_ml,zoversamp,prd);
    (T_D,m_out) = compute_nu_T_m_xy(dU_D,dV_D,U,V,thknss,dxu,dyu,dxv,dyv,dxc,dyc,ugrid,vgrid,post_taper,trim_ml,zoversamp,prd);
    (T_Z,m_out) = compute_nu_T_m_xy(dU_Z,dV_Z,U,V,thknss,dxu,dyu,dxv,dyv,dxc,dyc,ugrid,vgrid,post_taper,trim_ml,zoversamp,prd);

    return (T_D,T_Z,m_out);



def compute_nu_leith_sep(U,V,thknss,dxc,dyc,dxu,dyu,dxv,dyv,dxq,dyq,ugrid,vgrid,pgrid,post_taper=np.array([1,1,0]),
                     trim_ml=0,zoversamp=1,prd=np.array([1,1,1])):

    print('in compute_nu_leith_sep()')
