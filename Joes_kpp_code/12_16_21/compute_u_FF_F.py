...
def compute_u_FF_F_k(U,V,W,dx,dy,dz,post_taper=np.array([0,0,0]),trim_ml=0,prd=np.array([1,1,1])):

    print ('in compute_u_FF_F_k()')
    ...
    for kti in range(kt_offset,kti_max-kt_offset):
        ...
        hUp = np.real(np.fft.ifft(np.fft.ifft(FhUp,axis=0),axis=1)); hUb = hU - hUp;
        hVp = np.real(np.fft.ifft(np.fft.ifft(FhVp,axis=0),axis=1)); hVb = hV - hVp;
        ...
        F[kti,:,0] = np.reshape(rho0*dx*dy*dz*np.sum(np.sum(np.sum(dhUp*hUp + dhVp*hVp + 1.0*dh2KEdiv,axis=0,keepdims=True),axis=1,keepdims=True),axis=3,keepdims=True)/nt/LayerVol,(nz));
        F[kti,:,1] = np.reshape(rho0*dx*dy*dz*np.sum(np.sum(np.sum(dhUb*hUb + dhVb*hVb + 1.0*dh2KEdiv,axis=0,keepdims=True),axis=1,keepdims=True),axis=3,keepdims=True)/nt/LayerVol,(nz));
        F[kti,:,2] = np.reshape(rho0*dx*dy*dz*np.sum(np.sum(np.sum(dhUp*hUp + dhVp*hVp + 0.5*dh2KEdiv,axis=0,keepdims=True),axis=1,keepdims=True),axis=3,keepdims=True)/nt/LayerVol,(nz));
        F[kti,:,3] = np.reshape(rho0*dx*dy*dz*np.sum(np.sum(np.sum(dhUb*hUb + dhVb*hVb + 0.5*dh2KEdiv,axis=0,keepdims=True),axis=1,keepdims=True),axis=3,keepdims=True)/nt/LayerVol,(nz));

        print('comp. u_FF_F: kti = ' + str(kti) + ' of ' + str(kti_max) + ', time = ' + str(time.time() - t1)) # DEBUG
...
def compute_u_FF_F_m(U,V,W,dx,dy,dz,post_taper=np.array([0,0,0]),trim_ml=0,prd=np.array([1,1,1])):

    print ('in compute_u_FF_F_m()')
    ...
    for mti in range(mt_offset,mti_max-mt_offset):
        ...
        # Compute the spectral energy flux.  3 and 4 are corrected forward and backward, 1 and 2 are exact
        F[mti,:,:,0] = np.reshape(rho0*dx*dy*dz*np.sum(np.sum(dhUp*hUp + dhVp*hVp + 1.0*dh2KEdiv,axis=2,keepdims=True),axis=3,keepdims=True)/nt/ColumnVol,(nx,ny));
        F[mti,:,:,1] = np.reshape(rho0*dx*dy*dz*np.sum(np.sum(dhUb*hUb + dhVb*hVb + 1.0*dh2KEdiv,axis=2,keepdims=True),axis=3,keepdims=True)/nt/ColumnVol,(nx,ny));
        F[mti,:,:,2] = np.reshape(rho0*dx*dy*dz*np.sum(np.sum(dhUp*hUp + dhVp*hVp + 0.5*dh2KEdiv,axis=2,keepdims=True),axis=3,keepdims=True)/nt/ColumnVol,(nx,ny));
        F[mti,:,:,3] = np.reshape(rho0*dx*dy*dz*np.sum(np.sum(dhUb*hUb + dhVb*hVb + 0.5*dh2KEdiv,axis=2,keepdims=True),axis=3,keepdims=True)/nt/ColumnVol,(nx,ny));

        print('comp. u_FF_F: mti = ' + str(mti) + ' of ' + str(mti_max) + ', time = ' + str(time.time() - t1)) # DEBUG
