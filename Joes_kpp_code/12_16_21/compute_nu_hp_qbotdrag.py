...
def compute_nu_hp_qbotdrag_diag(U,V,Up,Vp,thknss,dxu,dyv,ugrid,vgrid,post_taper=np.array([1,1,0]),trim_ml=0,zoversamp=1,prd=np.array([1,1,1])):

    (dU,dV) = compute_nu_hp_qbotdrag(U,V,Up,Vp,thknss,ugrid,vgrid,post_taper=np.array([1,1,0]),trim_ml=0,zoversamp=1,prd=np.array([1,1,1]));

    return (dU,dV);

def compute_nu_hp_qbotdrag_diag_u(U,V,Up,Vp,thknss,dxu,dyv,ugrid,vgrid,post_taper=np.array([1,1,0]),trim_ml=0,zoversamp=1,prd=np.array([1,1,1])):
...
def compute_nu_hp_qbotdrag_k(U,V,Up,Vp,thknss,dxc,dyc,dxu,dyu,dxv,dyv,dxq,dyq,ugrid,vgrid,post_taper=np.array([1,1,0]),trim_ml=0,zoversamp=1,prd=np.array([1,1,1])):
...
def compute_nu_hp_qbotdrag_m(U,V,Up,Vp,thknss,dxc,dyc,dxu,dyu,dxv,dyv,dxq,dyq,ugrid,vgrid,post_taper=np.array([1,1,0]),trim_ml=0,zoversamp=1,prd=np.array([1,1,1])):
...
def compute_nu_hp_qbotdrag(U,V,Up,Vp,thknss,ugrid,vgrid,post_taper=np.array([1,1,0]),trim_ml=0,zoversamp=1,prd=np.array([1,1,1])):

    print ('in compute_nu_hp_qbotdrag()')
    ...
    Up = ft.pad_field_3D(Up,ng,prd); Vp = ft.pad_field_3D(Vp,ng,prd); 
    ...
    for i in range(gx0,gxn):
        for j in range(gy0,gyn):
            for t in range(0,nt):
                k = kbotW[i,j,0,t];
                dU[i,j,k,t] = -(Cd/thknssW[i,j,k,t])*((KE[i-1,j,k,t]+KE[i,j,k,t])**(0.5))*Up[i,j,k,t];
                k = kbotS[i,j,0,t];
                dV[i,j,k,t] = -(Cd/thknssS[i,j,k,t])*((KE[i,j-1,k,t]+KE[i,j,k,t])**(0.5))*Vp[i,j,k,t];
