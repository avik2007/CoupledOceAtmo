...
def compute_u_KE_k(U,V,W,dx,dy,dz,post_taper=np.array([0,0,0]),trim_ml=0,prd=np.array([1,1,1])):
    print ('in compute_u_KE_k()')
    (hU,hV,_) = ft.taper_filter_3D_uvw_u(U,V,W,post_taper);
    (T,k_out) = compute_u_T_k(hU,hV,hU,hV,dx,dy,dz,post_taper,trim_ml,prd); # takes filtered fields as arguments, diff from nu_T_k
    return (T,k_out);

def compute_u_KE_m(U,V,W,dx,dy,dz,post_taper=np.array([0,0,0]),trim_ml=0,prd=np.array([1,1,1])):
    print ('in compute_u_KE_m()')
    (hU,hV,_) = ft.taper_filter_3D_uvw_u(U,V,W,post_taper);
    (T,m_out) = compute_u_T_m(hU,hV,hU,hV,dx,dy,dz,post_taper,trim_ml,prd);
