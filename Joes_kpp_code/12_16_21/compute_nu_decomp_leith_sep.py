...
def compute_nu_decomp_leith_sep_m(U,V,Ubp,Vbp,Uhp,Vhp,thknss,dxc,dyc,dxu,dyu,dxv,dyv,dxq,dyq,ugrid,vgrid,pgrid,post_taper=np.array([1,1,0]),
                     trim_ml=0,zoversamp=1,prd=np.array([1,1,1])):

    nz = thknss.shape[2];
    mti_max = math.floor(nz/2-1); 
    T_D = np.zeros((mti_max,3));
    T_Z = np.zeros((mti_max,3));

    (dU_D,dV_D,dU_Z,dV_Z,_,_) = compute_nu_decomp_leith_sep(U,V,thknss,dxc,dyc,dxu,dyu,dxv,dyv,dxq,dyq,ugrid,vgrid,pgrid,post_taper,trim_ml,zoversamp,prd);
    (T_D_lp,m_out) = compute_nu_T_m(dU_D,dV_D,U-Ubp-Uhp,V-Vbp-Vhp,thknss,dxu,dyu,dxv,dyv,dxc,dyc,ugrid,vgrid,post_taper,trim_ml,zoversamp,prd);
    (T_Z_lp,m_out) = compute_nu_T_m(dU_Z,dV_Z,U-Ubp-Uhp,V-Vbp-Vhp,thknss,dxu,dyu,dxv,dyv,dxc,dyc,ugrid,vgrid,post_taper,trim_ml,zoversamp,prd);
    (T_D_bp,m_out) = compute_nu_T_m(dU_D,dV_D,Ubp,Vbp,thknss,dxu,dyu,dxv,dyv,dxc,dyc,ugrid,vgrid,post_taper,trim_ml,zoversamp,prd);
    (T_Z_bp,m_out) = compute_nu_T_m(dU_Z,dV_Z,Ubp,Vbp,thknss,dxu,dyu,dxv,dyv,dxc,dyc,ugrid,vgrid,post_taper,trim_ml,zoversamp,prd);
    (T_D_hp,m_out) = compute_nu_T_m(dU_D,dV_D,Uhp,Vhp,thknss,dxu,dyu,dxv,dyv,dxc,dyc,ugrid,vgrid,post_taper,trim_ml,zoversamp,prd);
    (T_Z_hp,m_out) = compute_nu_T_m(dU_Z,dV_Z,Uhp,Vhp,thknss,dxu,dyu,dxv,dyv,dxc,dyc,ugrid,vgrid,post_taper,trim_ml,zoversamp,prd);

    T_D[:,0:1] = T_D_lp; T_D[:,1:2] = T_D_bp; T_D[:,2:3] = T_D_hp;
    T_Z[:,0:1] = T_Z_lp; T_Z[:,1:2] = T_Z_bp; T_Z[:,2:3] = T_Z_hp;

    return (T_D,T_Z,m_out);

def compute_nu_decomp_leith_sep_m_xy(U,V,Ubp,Vbp,Uhp,Vhp,thknss,dxc,dyc,dxu,dyu,dxv,dyv,dxq,dyq,ugrid,vgrid,pgrid,post_taper=np.array([1,1,0]),
                     trim_ml=0,zoversamp=1,prd=np.array([1,1,1])):

    (nx,ny,nz) = thknss.shape[0:3];
    mti_max = math.floor(nz/2-1); 
    T_D = np.zeros((mti_max,nx,ny,3));
    T_Z = np.zeros((mti_max,nx,ny,3));

    (dU_D,dV_D,dU_Z,dV_Z,_,_) = compute_nu_decomp_leith_sep(U,V,thknss,dxc,dyc,dxu,dyu,dxv,dyv,dxq,dyq,ugrid,vgrid,pgrid,post_taper,trim_ml,zoversamp,prd);
    (T_D_lp,m_out) = compute_nu_T_m_xy(dU_D,dV_D,U-Ubp-Uhp,V-Vbp-Vhp,thknss,dxu,dyu,dxv,dyv,dxc,dyc,ugrid,vgrid,post_taper,trim_ml,zoversamp,prd);
    (T_Z_lp,m_out) = compute_nu_T_m_xy(dU_Z,dV_Z,U-Ubp-Uhp,V-Vbp-Vhp,thknss,dxu,dyu,dxv,dyv,dxc,dyc,ugrid,vgrid,post_taper,trim_ml,zoversamp,prd);
    (T_D_bp,m_out) = compute_nu_T_m_xy(dU_D,dV_D,Ubp,Vbp,thknss,dxu,dyu,dxv,dyv,dxc,dyc,ugrid,vgrid,post_taper,trim_ml,zoversamp,prd);
    (T_Z_bp,m_out) = compute_nu_T_m_xy(dU_Z,dV_Z,Ubp,Vbp,thknss,dxu,dyu,dxv,dyv,dxc,dyc,ugrid,vgrid,post_taper,trim_ml,zoversamp,prd);
    (T_D_hp,m_out) = compute_nu_T_m_xy(dU_D,dV_D,Uhp,Vhp,thknss,dxu,dyu,dxv,dyv,dxc,dyc,ugrid,vgrid,post_taper,trim_ml,zoversamp,prd);
    (T_Z_hp,m_out) = compute_nu_T_m_xy(dU_Z,dV_Z,Uhp,Vhp,thknss,dxu,dyu,dxv,dyv,dxc,dyc,ugrid,vgrid,post_taper,trim_ml,zoversamp,prd);

    T_D[:,:,:,0] = T_D_lp; T_D[:,:,:,1] = T_D_bp; T_D[:,:,:,2] = T_D_hp;
    T_Z[:,:,:,0] = T_Z_lp; T_Z[:,:,:,1] = T_Z_bp; T_Z[:,:,:,2] = T_Z_hp;

    return (T_D,T_Z,m_out);

def compute_nu_decomp_leith_sep(U,V,thknss,dxc,dyc,dxu,dyu,dxv,dyv,dxq,dyq,ugrid,vgrid,pgrid,post_taper=np.array([1,1,0]),
                     trim_ml=0,zoversamp=1,prd=np.array([1,1,1])):

    print('in compute_nu_decomp_leith_sep()')
