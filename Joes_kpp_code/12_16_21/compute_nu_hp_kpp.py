...
def compute_nu_hp_kpp_sep_diag(U,V,Up,Vp,rho,pot_rho_down,thknss,dxc,dyc,dxu,dyu,dxv,dyv,dxq,dyq,ugrid,vgrid,pgrid,post_taper=np.array([1,1,0]),
                     trim_ml=0,zoversamp=1,prd=np.array([1,1,1])):
    (_,_,dUb,dUc,dUs,dVb,dVc,dVs,_,_,_) = compute_nu_hp_kpp(U,V,rho,pot_rho_down,thknss,dxc,dyc,dxu,dyu,dxv,dyv,dxq,dyq,ugrid,vgrid,pgrid,post_taper,trim_ml,zoversamp,prd);

    return (dUb,dVb,dUc,dVc,dUs,dVs);

def compute_nu_hp_kpp_sep_diag_u(U,V,Up,Vp,rho,pot_rho_down,thknss,dxc,dyc,dxu,dyu,dxv,dyv,dxq,dyq,ugrid,vgrid,pgrid,post_taper=np.array([1,1,0]),
                     trim_ml=0,zoversamp=1,prd=np.array([1,1,1])):
    (_,_,dUb,dUc,dUs,dVb,dVc,dVs,_,_,_) = compute_nu_hp_kpp(U,V,rho,pot_rho_down,thknss,dxc,dyc,dxu,dyu,dxv,dyv,dxq,dyq,ugrid,vgrid,pgrid,post_taper,trim_ml,zoversamp,prd);
...
def compute_nu_kpp_diag_nu_bar(U,V,rho,pot_rho_down,thknss,dxc,dyc,dxu,dyu,dxv,dyv,dxq,dyq,ugrid,vgrid,pgrid,post_taper=np.array([1,1,0]),
                     trim_ml=0,zoversamp=1,prd=np.array([1,1,1])):
    (_,_,_,_,_,_,_,_,nu0,nu1,nu2) = compute_nu_hp_kpp(U,V,rho,pot_rho_down,thknss,dxc,dyc,dxu,dyu,dxv,dyv,dxq,dyq,ugrid,vgrid,pgrid,post_taper,0,zoversamp,prd);

    # perform spatial average within function.  Need to do time average outside.
    nu_bar = np.zeros((3));
    nu_bar[0] = ft.get_volume_average(nu0,thknss,dxc,dyc)
    nu_bar[1] = ft.get_volume_average(nu1,thknss,dxc,dyc)
    nu_bar[2] = ft.get_volume_average(nu2,thknss,dxc,dyc)

    return nu_bar;

def compute_nu_hp_kpp_k(U,V,Up,Vp,rho,pot_rho_down,thknss,dxc,dyc,dxu,dyu,dxv,dyv,dxq,dyq,ugrid,vgrid,pgrid,post_taper=np.array([1,1,0]),
                     trim_ml=0,zoversamp=1,prd=np.array([1,1,1])):
    (dU,dV,_,_,_,_,_,_,_,_,_) = compute_nu_hp_kpp(U,V,rho,pot_rho_down,thknss,dxc,dyc,dxu,dyu,dxv,dyv,dxq,dyq,ugrid,vgrid,pgrid,post_taper,trim_ml,zoversamp,prd);
    (T,k_out) = compute_nu_T_k(dU,dV,Up,Vp,thknss,dxu,dyu,dxv,dyv,dxc,dyc,ugrid,vgrid,post_taper,0,zoversamp,prd);
    return (T,k_out);

def compute_nu_hp_kpp_m(U,V,Up,Vp,rho,pot_rho_down,thknss,dxc,dyc,dxu,dyu,dxv,dyv,dxq,dyq,ugrid,vgrid,pgrid,post_taper=np.array([1,1,0]),
                     trim_ml=0,zoversamp=1,prd=np.array([1,1,1])):
    (dU,dV,_,_,_,_,_,_,_,_,_) = compute_nu_hp_kpp(U,V,rho,pot_rho_down,thknss,dxc,dyc,dxu,dyu,dxv,dyv,dxq,dyq,ugrid,vgrid,pgrid,post_taper,trim_ml,zoversamp,prd);
    (T,m_out) = compute_nu_T_m(dU,dV,Up,Vp,thknss,dxu,dyu,dxv,dyv,dxc,dyc,ugrid,vgrid,post_taper,0,zoversamp,prd);
    return (T,m_out);

def compute_nu_hp_kpp_k_z(U,V,Up,Vp,rho,pot_rho_down,thknss,dxc,dyc,dxu,dyu,dxv,dyv,dxq,dyq,ugrid,vgrid,pgrid,post_taper=np.array([1,1,0]),
                     trim_ml=0,zoversamp=1,prd=np.array([1,1,1])):
    (dU,dV,_,_,_,_,_,_,_,_,_) = compute_nu_hp_kpp(U,V,rho,pot_rho_down,thknss,dxc,dyc,dxu,dyu,dxv,dyv,dxq,dyq,ugrid,vgrid,pgrid,post_taper,trim_ml,zoversamp,prd);
    (T,k_out) = compute_nu_T_k_z(dU,dV,Up,Vp,thknss,dxu,dyu,dxv,dyv,dxc,dyc,ugrid,vgrid,post_taper,0,zoversamp,prd);
    return (T,k_out);

def compute_nu_hp_kpp_m_xy(U,V,Up,Vp,rho,pot_rho_down,thknss,dxc,dyc,dxu,dyu,dxv,dyv,dxq,dyq,ugrid,vgrid,pgrid,post_taper=np.array([1,1,0]),
                     trim_ml=0,zoversamp=1,prd=np.array([1,1,1])):
    (dU,dV,_,_,_,_,_,_,_,_,_) = compute_nu_hp_kpp(U,V,rho,pot_rho_down,thknss,dxc,dyc,dxu,dyu,dxv,dyv,dxq,dyq,ugrid,vgrid,pgrid,post_taper,trim_ml,zoversamp,prd);
    (T,m_out) = compute_nu_T_m_xy(dU,dV,Up,Vp,thknss,dxu,dyu,dxv,dyv,dxc,dyc,ugrid,vgrid,post_taper,0,zoversamp,prd);
    return (T,m_out);

def compute_nu_hp_kpp_sep_k(U,V,Up,Vp,rho,pot_rho_down,thknss,dxc,dyc,dxu,dyu,dxv,dyv,dxq,dyq,ugrid,vgrid,pgrid,post_taper=np.array([1,1,0]),
                     trim_ml=0,zoversamp=1,prd=np.array([1,1,1])):
    (_,_,dUb,dUc,dUs,dVb,dVc,dVs,_,_,_) = compute_nu_hp_kpp(U,V,rho,pot_rho_down,thknss,dxc,dyc,dxu,dyu,dxv,dyv,dxq,dyq,ugrid,vgrid,pgrid,post_taper,trim_ml,zoversamp,prd);

    (Ts,_) = compute_nu_T_k(dUs,dVs,Up,Vp,thknss,dxu,dyu,dxv,dyv,dxc,dyc,ugrid,vgrid,post_taper,0,zoversamp,prd);
    (Tb,k_out) = compute_nu_T_k(dUb,dVb,Up,Vp,thknss,dxu,dyu,dxv,dyv,dxc,dyc,ugrid,vgrid,post_taper,0,zoversamp,prd);
    (Tc,_) = compute_nu_T_k(dUc,dVc,Up,Vp,thknss,dxu,dyu,dxv,dyv,dxc,dyc,ugrid,vgrid,post_taper,0,zoversamp,prd);
    return (Tb,Tc,Ts,k_out);

def compute_nu_hp_kpp_sep_m(U,V,Up,Vp,rho,pot_rho_down,thknss,dxc,dyc,dxu,dyu,dxv,dyv,dxq,dyq,ugrid,vgrid,pgrid,post_taper=np.array([1,1,0]),
                     trim_ml=0,zoversamp=1,prd=np.array([1,1,1])):
    (_,_,dUb,dUc,dUs,dVb,dVc,dVs,_,_,_) = compute_nu_hp_kpp(U,V,rho,pot_rho_down,thknss,dxc,dyc,dxu,dyu,dxv,dyv,dxq,dyq,ugrid,vgrid,pgrid,post_taper,trim_ml,zoversamp,prd);

    (Ts,_) = compute_nu_T_m(dUs,dVs,Up,Vp,thknss,dxu,dyu,dxv,dyv,dxc,dyc,ugrid,vgrid,post_taper,0,zoversamp,prd);
    (Tb,m_out) = compute_nu_T_m(dUb,dVb,Up,Vp,thknss,dxu,dyu,dxv,dyv,dxc,dyc,ugrid,vgrid,post_taper,0,zoversamp,prd);
    (Tc,_) = compute_nu_T_m(dUc,dVc,Up,Vp,thknss,dxu,dyu,dxv,dyv,dxc,dyc,ugrid,vgrid,post_taper,0,zoversamp,prd);
    return (Tb,Tc,Ts,m_out);

def compute_nu_hp_kpp_sep_k_z(U,V,Up,Vp,rho,pot_rho_down,thknss,dxc,dyc,dxu,dyu,dxv,dyv,dxq,dyq,ugrid,vgrid,pgrid,post_taper=np.array([1,1,0]),trim_ml=0,zoversamp=1,prd=np.array([1,1,1])):

    (_,_,dUb,dUc,dUs,dVb,dVc,dVs,_,_,_) = compute_nu_hp_kpp(U,V,rho,pot_rho_down,thknss,dxc,dyc,dxu,dyu,dxv,dyv,dxq,dyq,ugrid,vgrid,pgrid,post_taper,trim_ml,zoversamp,prd);

    (Ts,_) = compute_nu_T_k_z(dUs,dVs,Up,Vp,thknss,dxu,dyu,dxv,dyv,dxc,dyc,ugrid,vgrid,post_taper,0,zoversamp,prd);
    (Tb,k_out) = compute_nu_T_k_z(dUb,dVb,Up,Vp,thknss,dxu,dyu,dxv,dyv,dxc,dyc,ugrid,vgrid,post_taper,0,zoversamp,prd);
    (Tc,_) = compute_nu_T_k_z(dUc,dVc,Up,Vp,thknss,dxu,dyu,dxv,dyv,dxc,dyc,ugrid,vgrid,post_taper,0,zoversamp,prd);
    return (Tb,Tc,Ts,k_out);

def compute_nu_hp_kpp_sep_m_xy(U,V,Up,Vp,rho,pot_rho_down,thknss,dxc,dyc,dxu,dyu,dxv,dyv,dxq,dyq,ugrid,vgrid,pgrid,post_taper=np.array([1,1,0]),trim_ml=0,zoversamp=1,prd=np.array([1,1,1])):

    (_,_,dUb,dUc,dUs,dVb,dVc,dVs,_,_,_) = compute_nu_hp_kpp(U,V,rho,pot_rho_down,thknss,dxc,dyc,dxu,dyu,dxv,dyv,dxq,dyq,ugrid,vgrid,pgrid,post_taper,trim_ml,zoversamp,prd);

    (Ts,_) = compute_nu_T_m_xy(dUs,dVs,Up,Vp,thknss,dxu,dyu,dxv,dyv,dxc,dyc,ugrid,vgrid,post_taper,0,zoversamp,prd);
    (Tb,m_out) = compute_nu_T_m_xy(dUb,dVb,Up,Vp,thknss,dxu,dyu,dxv,dyv,dxc,dyc,ugrid,vgrid,post_taper,0,zoversamp,prd);
    (Tc,_) = compute_nu_T_m_xy(dUc,dVc,Up,Vp,thknss,dxu,dyu,dxv,dyv,dxc,dyc,ugrid,vgrid,post_taper,0,zoversamp,prd);
    return (Tb,Tc,Ts,m_out);
...
def compute_nu_hp_kpp(U,V,rho,pot_rho_down,thknss,dxc,dyc,dxu,dyu,dxv,dyv,dxq,dyq,ugrid,vgrid,pgrid,post_taper=np.array([1,1,0]),
                     trim_ml=0,zoversamp=1,prd=np.array([1,1,1])):

    print ('in compute_nu_hp_kpp()')
