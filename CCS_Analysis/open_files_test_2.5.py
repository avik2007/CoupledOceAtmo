...
import params_500mSNAPS_offset as p
...
def main(grid,maxlevel,c):
    ...
    #i=c.timesteps[0]
    timestamps = np.arange(798408, 798408+24*144, 144)
    i=timestamps[0]
    print(i)
    """
    ...
    for i in timestamps[1:final_time]:
        print(i)
        """
        t = c.loadding_3D_data(p.dirc+'THETA.%010i.data'%i,maxlevel,'tracer')
        t = t[:,grid['ilnc'][0]:grid['ilnc'][1],grid['iltc'][0]:grid['iltc'][1]]
...
"""
    ...
    KPPviscA= np.expand_dims(KPPviscA, 0)
        ...
        """
        sa = c.loadding_3D_data(p.dirc+'SALT.%010i.data'%i,maxlevel,'tracer')
